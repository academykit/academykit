import { Box, Button, Card, Container, Group, Loader, Text } from "@mantine/core";
import { showNotification } from "@mantine/notifications";
import { AssessmentStatus } from "@utils/enums";
import RoutePath from "@utils/routeConstants";
import { useGetSingleAssessment, useUpdateAssessmentStatus } from "@utils/services/assessmentService";
      ...
      <Container fluid>
        <Group className={classes.group}>
          <Card shadow={"sm"} className={classes.right}>
            ...
            <Button mt={20} component={Link} onClick={onUpdatePublish} to={RoutePath.manageAssessment.edit(id).route}>
              {t("update_assessment")}
            </Button>
          </Card>
          <Text>{t("or")}</Text>
          <Card shadow={"sm"} className={classes.left}>
            ...
            <Button loading={assessmentStatus.isLoading} onClick={onPublish} disabled={assessmentButton}>
              {t("update_publish")}
            </Button>
          </Card>
        </Group>
      ...
      <Container fluid>
        <Group className={classes.group}>
          <Card shadow={"sm"} className={classes.right}>
            ...
            <Button mt={20} component={Link} to={RoutePath.manageAssessment.edit(id).route}>
              {t("continue_edit")}
            </Button>
          </Card>
          <Text>{t("or")}</Text>
