...
import { type IGroupAttachmentItems, useGroupAttachment, useRemoveGroupAttachment } from "@utils/services/groupService";
import moment from "moment";
import { useState } from "react";
import { useTranslation } from "react-i18next";
import { useParams } from "react-router-dom";

const GroupAttachment = ({ searchParams, pagination, searchComponent }: IWithSearchPagination) => {
  const { id } = useParams();
  ...
  const Rows = ({ item }: { item: IGroupAttachmentItems }) => {
    ...
    const auth = useAuth();
    const removeAttachment = useRemoveGroupAttachment(id as string, item.id, searchParams);
    ...
    return (
      <Table.Tr key={item.id}>
        ...
        <Table.Td>{moment(item.createdOn + "Z").format("YYYY-MM-DD HH:mm:ss")}</Table.Td>
          ...
          <Flex>
            ...
            {auth?.auth && Number(auth?.auth?.role) !== UserRole.Trainee && (
              <Tooltip label={t("delete_attachment")}>
                <ActionIcon variant="subtle" onClick={() => setDeleteAttachment(item.id)}>
                  <IconTrash color="red" />
                </ActionIcon>
              </Tooltip>
            )}
  ...
  return (
    <>
      ...
      >
        {opened && <AddAssignment close={() => setOpened(false)} search={searchParams} />}
      </Modal>
      <Container fluid>
        ...
        {getGroupAttachment.data && getGroupAttachment.data?.items.length >= 1 ? (
          <ScrollArea>
            <Paper mt={10}>
              <Table
              ...
              {getGroupAttachment.data &&
                ...
                pagination(getGroupAttachment.data?.totalPage, getGroupAttachment.data.items.length)}
            </Paper>
          </ScrollArea>
        ) : (
