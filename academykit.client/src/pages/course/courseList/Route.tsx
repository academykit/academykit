...
import { Box, Button, Container, Group, Loader, Tabs, Title } from "@mantine/core";
...
const CoursePage = lazyWithRetry(() => import("@pages/course/courseList/course"));
const CreatedCoursePage = lazyWithRetry(() => import("@pages/course/courseList/createdCourses"));
const CompletedCourseList = lazyWithRetry(() => import("@pages/course/courseList/completedCourse"));
      ...
      <Box my={10} style={{ justifyContent: "space-between", alignItems: "center" }}>
        ...
        <Tabs my={20} value={location.pathname} onChange={(value) => navigate(`${value}`)}>
          <Tabs.List>
            ...
            <Tabs.Tab value="/trainings/list/completed">{t("completed_trainings")}</Tabs.Tab>
            {Number(role) <= UserRole.Admin && (
              <Tabs.Tab value="/trainings/list/review">{t("review_trainings")}</Tabs.Tab>
            )}
          </Tabs.List>
        </Tabs>
      </Box>
