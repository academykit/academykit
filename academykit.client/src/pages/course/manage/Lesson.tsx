...
import { Anchor, Badge, Box, Button, Center, Loader, Paper, ScrollArea, Table, Tooltip } from "@mantine/core";
import { IconEye } from "@tabler/icons-react";
import { LessonType } from "@utils/enums";
import RoutePath from "@utils/routeConstants";
import { type ILessonStats, useGetLessonStatistics } from "@utils/services/manageCourseService";
  ...
  return (
    <Table.Tr key={item?.id}>
      <Table.Td style={{ maxWidth: "200px" }}>
        <Anchor component={Link} to={`${RoutePath.classes}/${course_id}/${item.slug}`}>
          {item.name}
        </Anchor>
      </Table.Td>
      <Table.Td>{t(`${LessonType[item.lessonType]}`)}</Table.Td>
      <Table.Td>
        <ProgressBar total={item?.enrolledStudent} positive={item?.lessonWatched} />
      </Table.Td>
      <Table.Td>
        <Center>
          {item?.isMandatory ? (
...
function TableReviews({ searchParams, pagination }: IWithSearchPagination) {
  ...
  if (getLessonStatistics.data && getLessonStatistics.data.items && getLessonStatistics.data?.items?.length === 0)
  ...
  return (
    <ScrollArea>
      ...
      {getLessonStatistics.data &&
        ...
        pagination(getLessonStatistics.data?.totalPage, getLessonStatistics.data.items.length)}
    </ScrollArea>
  );
}
