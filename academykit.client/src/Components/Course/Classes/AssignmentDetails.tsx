...
const AssignmentDetails = ({ lesson }: { lesson: ICourseLesson }) => {
  ...
  const lesson_start_date = moment.utc(lesson.startDate, "YYYY-MM-DD[T]HH:mm[Z]");
  ...
  return (
    <Group style={{ flexDirection: "column" }}>
      ...
      {lesson.hasReviewedAssignment ? (
        <Button component={Link} to={RoutePath.assignment.result(lesson.id, user?.auth?.id).route}>
          {t("view_result")}
        </Button>
      ) : lesson.assignmentExpired ? (
        <Text>{t("assignment_expired")}</Text>
      ) : !lesson.startDate || current_time.isAfter(lesson_start_date) ? (
        <Button component={Link} to={RoutePath.assignment.details(lesson.id).route}>
          {lesson.isCompleted && lesson.isTrainee ? t("resubmit") : lesson.isTrainee && t("start_assignment")}
        ...
        <Text>
          ...
          {moment(lesson.startDate).add(5, "hours").add(45, "minutes").format("MMM DD, YYYY hh:mm A")}
        </Text>
      )}
      {lesson.assignmentReview && (
        <Box mb={20} style={{ color: theme.white }}>
          ...
          <Group style={{ alignItems: "start" }} mt={20}>
            <UserShortProfile user={lesson.assignmentReview.teacher} size={"sm"} color="#C1C2C5" />
            <Paper
              style={{
