...
import { type IFeedbackQuestions, type IFeedbackSubmission, useFeedbackSubmission } from "@utils/services/feedbackService";
  ...
  const handleSubmit = async (values: IFeedbackQuestions[]) => {
    ...
    values.forEach((x) => {
      ...
      if (x.feedbackQuestionOptions) {
        data["selectedOption"] = x.feedbackQuestionOptions.filter((y) => y.isSelected).map((y) => y.id);
      }
      finalData.push(data);
    });
    ...
    <form onSubmit={form.onSubmit(handleSubmit)}>
      {item.map((x, currentIndex) => (
        <Card key={x.id} shadow="sm" my={10} withBorder>
          ...
          {x.type === FeedbackType.MultipleChoice && x?.feedbackQuestionOptions && (
            <FeedbackCheckBoxType options={x?.feedbackQuestionOptions} currentIndex={currentIndex} form={form} />
          )}
          {x.type === FeedbackType.SingleChoice && x?.feedbackQuestionOptions && (
            <FeedbackRadio options={x?.feedbackQuestionOptions} currentIndex={currentIndex} form={form} />
          )}
          {x.type === FeedbackType.Subjective && <FeedbackSubjective form={form} currentIndex={currentIndex} />}
          {x.type === FeedbackType.Rating && (
            <FeedbackRating form={form} currentIndex={currentIndex} readOnly={false} />
          )}
        </Card>
      ))}
      {item[0].isTrainee && (
