...
export const getCurrentGroup = (currentLocation: string, group: IRouteGroup[]) => {
  const match = group.find((item) => currentLocation == item.route.split("/")[2]);
  return match ? match.group : "";
};

export const getCurrentNavGroup = (currentLocation: string, group: IRouteGroup[]) => {
