...
const postAttendance = async ({ identity }: { identity: string }) =>
  await httpClient.post<ResponseData>(api.physicalTraining.attendance(identity), {});
...
export const useReviewAttendance = (courseId: string, lessonId: string, userId: string) => {
  const queryClient = useQueryClient();
