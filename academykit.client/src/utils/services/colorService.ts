export type BrandingThemeType = [string, string, string, string, string, string, string, string, string, string];

const mixColors = (color1: string, color2: string, percent: number) => {
  const p = percent / 100;
  const R1 = Number.parseInt(color1.substring(0, 2), 16);
  const G1 = Number.parseInt(color1.substring(2, 4), 16);
  const B1 = Number.parseInt(color1.substring(4, 6), 16);

  const R2 = Number.parseInt(color2.substring(0, 2), 16);
  const G2 = Number.parseInt(color2.substring(2, 4), 16);
  const B2 = Number.parseInt(color2.substring(4, 6), 16);
