namespace Lingtren.Application.Common.Validators
{
    ...
    public class ZoomSettingValidator : AbstractValidator<ZoomSettingRequestModel>
    {
        ...
        {
            RuleFor(x => x.OAuthAccountId).NotNull().NotEmpty().WithMessage(context => stringLocalizer.GetString("OAuthAccountIdRequired")).MaximumLength(100).WithMessage(context => stringLocalizer.GetString("OAuthAccountIdLength100"));
             RuleFor(x => x.OAuthClientId).NotNull().NotEmpty().WithMessage(context => stringLocalizer.GetString("OAuthClientIdRequired")).MaximumLength(100).WithMessage(context => stringLocalizer.GetString("OAuthClientIdLength100"));
            RuleFor(x => x.OAuthClientSecret).NotNull().NotEmpty().WithMessage(context => stringLocalizer.GetString("OAuthClientSecretRequired")).MaximumLength(100).WithMessage(context => stringLocalizer.GetString("OAuthClientSecretLength100"));
