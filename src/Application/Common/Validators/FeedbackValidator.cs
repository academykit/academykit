namespace Lingtren.Application.Common.Validators
{
    ...
    public class FeedbackValidator : AbstractValidator<FeedbackRequestModel>
    {
        public FeedbackValidator(IStringLocalizer<ValidatorLocalizer> stringLocalizer)
        {
            ...
            RuleFor(x => x.Name).NotEmpty().NotNull().WithMessage(context => stringLocalizer.GetString("FeedbackNameRequired")).MaximumLength(500)
                        .WithMessage(context => stringLocalizer.GetString("NameLength500"));
            RuleFor(x => x.Type).NotNull().NotEmpty().WithMessage(context => stringLocalizer.GetString("FeedbackTypeRequired")).IsInEnum().WithMessage(context => stringLocalizer.GetString("InvalidFeedBackType"));
            ...
            RuleFor(x => x.Answers).Must(x => x.Count >= 2).WithMessage(context => stringLocalizer.GetString("OptionMoreThanOne"))
                                    .When(x => x.Type == FeedbackTypeEnum.SingleChoice || x.Type == FeedbackTypeEnum.MultipleChoice);
            RuleForEach(x => x.Answers).SetValidator(new FeedbackQuestionOptionValidator(stringLocalizer))
                                    .When(x => x.Type == FeedbackTypeEnum.SingleChoice || x.Type == FeedbackTypeEnum.MultipleChoice);
        }
    }
