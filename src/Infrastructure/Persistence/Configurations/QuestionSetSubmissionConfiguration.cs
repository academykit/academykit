namespace Lingtren.Infrastructure.Persistence.Configurations
{
    ...
    public class QuestionSetSubmissionConfiguration : IEntityTypeConfiguration<QuestionSetSubmission>
    {
        public void Configure(EntityTypeBuilder<QuestionSetSubmission> builder)
        {
            ...
            builder.Property(x => x.IsSubmissionError).HasColumnName("is_submission_error").HasDefaultValue(false);
            builder.Property(x => x.SubmissionErrorMessage).HasColumnName("submission_error_message").HasColumnType("VARCHAR(250)").HasMaxLength(250).IsRequired(false);
            ...
            builder.HasMany(x => x.QuestionSetResults).WithOne(x => x.QuestionSetSubmission).HasForeignKey(x => x.QuestionSetSubmissionId).OnDelete(DeleteBehavior.NoAction);
            builder.HasMany(x => x.QuestionSetSubmissionAnswers).WithOne(x => x.QuestionSetSubmission).HasForeignKey(x => x.QuestionSetSubmissionId).OnDelete(DeleteBehavior.NoAction);
        }
    }
}
