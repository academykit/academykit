namespace Lingtren.Infrastructure.Persistence.Configurations
{
    ...
    public class CourseConfiguration : IEntityTypeConfiguration<Course>
    {
        public void Configure(EntityTypeBuilder<Course> builder)
        {
            ...
            builder.HasMany(x => x.Signatures).WithOne(x => x.Course).HasForeignKey(x => x.CourseId).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(x => x.CourseCertificate).WithOne(x => x.Course).HasForeignKey<CourseCertificate>(x => x.CourseId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}