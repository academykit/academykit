namespace Lingtren.Infrastructure.Persistence.Configurations
{
    ...
    public class QuestionSetConfiguration : IEntityTypeConfiguration<QuestionSet>
    {
        public void Configure(EntityTypeBuilder<QuestionSet> builder)
        {
            ...
            builder.Property(x => x.Slug).HasColumnName("slug").HasColumnType("VARCHAR(520)").HasMaxLength(520).IsRequired();
            builder.Property(x => x.ThumbnailUrl).HasColumnName("thumbnail_url").HasMaxLength(500).IsRequired(false);
            ...
            builder.Property(x => x.PassingWeightage).HasColumnName("passing_weightage").HasColumnType("decimal(10,4)");
            ...
            builder.HasOne(x => x.Lesson).WithOne(x => x.QuestionSet).HasForeignKey<Lesson>(x => x.QuestionSetId).OnDelete(DeleteBehavior.NoAction);
            builder.HasMany(x => x.QuestionSetResults).WithOne(x => x.QuestionSet).HasForeignKey(x => x.QuestionSetId).OnDelete(DeleteBehavior.NoAction);
            builder.HasMany(x => x.QuestionSetSubmissions).WithOne(x => x.QuestionSet).HasForeignKey(x => x.QuestionSetId).OnDelete(DeleteBehavior.NoAction);
        }
    }
}