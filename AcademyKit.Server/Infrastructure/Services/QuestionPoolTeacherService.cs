namespace AcademyKit.Infrastructure.Services
{
    ...
    public class QuestionPoolTeacherService
        : BaseGenericService<QuestionPoolTeacher, QuestionPoolTeacherBaseSearchCriteria>,
            IQuestionPoolTeacherService
    {
        public QuestionPoolTeacherService(
        ...
        )
        {
            try
            {
                var questionPool = await _unitOfWork
                    ...
                    .GetFirstOrDefaultAsync(
                        ...
                        include: src => src.Include(y => y.QuestionPoolTeachers)
                    )
                ...
                var user = questionPool?.QuestionPoolTeachers.FirstOrDefault(x =>
                    x.UserId == userId
                );
                if (user == null)
                {
                    throw new EntityNotFoundException(
                        _localizer.GetString("QuestionPoolUserNotFound")
                    );
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "An error occurred while attempting to assign role to question pool."
                );
                throw ex is ServiceException
                    ? ex
                    : new ServiceException(_localizer.GetString("AssignRoleQuestionPoolError"));
            }
        }
        ...
        protected override Expression<Func<QuestionPoolTeacher, bool>> PredicateForIdOrSlug(
            string identity
        )
        {
        ...
        private static bool ValidateQuestionPoolMaintainer(
        ...
        )
        {
            ...
            var currentUser = question?.QuestionPoolTeachers.FirstOrDefault(x =>
                x.UserId == currentUserId && x.Role == PoolRole.Creator
            );
        ...
        )
        {
            ...
            {
                _logger.LogWarning(
                    "User with id : {userId} cannot remove own-self from question pool teacher with pool id : {poolId}.",
                    currentUserId,
                    teacher.QuestionPoolId
                );
                throw new ForbiddenException(_localizer.GetString("UserCanNotRemoveSameUser"));
            }

            var questionPool = await ValidateAndGetQuestionPool(
                    currentUserId,
                    questionPoolIdentity: teacher.QuestionPoolId.ToString()
                )
                .ConfigureAwait(false);
            if (questionPool.CreatedBy == teacher.UserId)
            {
                _logger.LogWarning(
                    "QuestionPool with Id {id} creator User Id {userId} can't be delete from questionPool teacher.",
                    questionPool.Id,
                    teacher.UserId
                );
        ...
        protected override async Task CreatePreHookAsync(QuestionPoolTeacher entity)
        {
            var questionPool = await ValidateAndGetQuestionPool(
                    entity.CreatedBy,
                    questionPoolIdentity: entity.QuestionPoolId.ToString()
                )
                .ConfigureAwait(false);
            if (questionPool.CreatedBy == entity.UserId)
            {
                _logger.LogWarning(
                    "QuestionPool with Id : {id} creator User Id : {userId} can't be questionPool teacher.",
                    questionPool.Id,
                    entity.UserId
                );
            ...
            {
                var poolRole = questionPool
                    .QuestionPoolTeachers.FirstOrDefault(p => p.UserId == entity.UserId)
                    ?.Role;
                _logger.LogWarning(
                    "User with Id {userId} is already question pool {role} of question pool with Id  : {id}.",
                    entity.UserId,
                    poolRole,
                    questionPool.Id
                );
        ...
        protected override Expression<Func<QuestionPoolTeacher, bool>> ConstructQueryConditions(
            Expression<Func<QuestionPoolTeacher, bool>> predicate,
            QuestionPoolTeacherBaseSearchCriteria criteria
        )
        {
            if (!string.IsNullOrWhiteSpace(criteria.QuestionPoolIdentity))
            {
                predicate = predicate.And(x =>
                    x.QuestionPool.Slug == criteria.QuestionPoolIdentity
                    || x.QuestionPool.Id.ToString() == criteria.QuestionPoolIdentity
                );
            }
        ...
        protected override IIncludableQueryable<
            QuestionPoolTeacher,
            object
        > IncludeNavigationProperties(IQueryable<QuestionPoolTeacher> query)
        {
